...
Service::group(array('prefix' => 'v1'), function()
{
	...
	Service::put('account/(:any)', array('json', 'xml'), function(Service $service, $uuid = null)
	{
		if( ! is_uuid($uuid))
		{
	...
	Service::get('account/total', array('json', 'xml'), function(Service $service)
	{
	...
	Service::delete('account/(:any)', array('json', 'xml'), function(Service $service, $uuid)
	{
		// Find the account we are updating
		$account = Account::find($uuid);
