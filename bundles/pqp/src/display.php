...
$pqpoutput .=  '<div id="pqp-container" class="pQp" style="display:none; left: 0; position: absolute;">';
...
$pqpoutput .=  '<div id="pqp-console" class="pqp-box">';

if($logCount ==  0) {
	$pqpoutput .=  '<h3>This panel has no log items.</h3>';
}
else {
	$pqpoutput .=  '<table class="side" cellspacing="0">
		<tr>
		...
		foreach($output['logs']['console'] as $log) {
			$pqpoutput .=  '<tr class="log-'.$log['type'].'">
				<td class="type">'.$log['type'].'</td>
				<td class="'.$class.'">';
			if($log['type'] == 'log') {
				$pqpoutput .=  '<div><pre>'.$log['data'].'</pre></div>';
			}
			elseif($log['type'] == 'memory') {
				$pqpoutput .=  '<div><pre>'.$log['data'].'</pre> <em>'.$log['dataType'].'</em>: '.$log['name'].' </div>';
			}
			elseif($log['type'] == 'speed') {
				$pqpoutput .=  '<div><pre>'.$log['data'].'</pre> <em>'.$log['name'].'</em></div>';
			}
			elseif($log['type'] == 'error') {
				$pqpoutput .=  '<div><em>Line '.$log['line'].'</em> : '.$log['data'].' <pre>'.$log['file'].'</pre></div>';
			}
...
$pqpoutput .=  '<div id="pqp-speed" class="pqp-box">';
...
$pqpoutput .=  '<div id="pqp-queries" class="pqp-box">';
...
$pqpoutput .=  '<div id="pqp-memory" class="pqp-box">';
...
if($output['logs']['memoryCount'] ==  0) {
	...
	$pqpoutput .=  '<table class="side" cellspacing="0">
		...
		foreach($output['logs']['console'] as $log) {
			if($log['type'] == 'memory') {
				$pqpoutput .=  '<tr class="log-'.$log['type'].'">';
				$pqpoutput .=  '<td class="'.$class.'"><b>'.$log['data'].'</b> <em>'.$log['dataType'].'</em>: '.$log['name'].'</td>';
				$pqpoutput .=  '</tr>';
				if($class == '') $class = 'alt';
				else $class = '';
			}
...
$pqpoutput .=  '<div id="pqp-files" class="pqp-box">';
...
if($output['fileTotals']['count'] ==  0) {
	...
	$pqpoutput .=  '<table class="side" cellspacing="0">
		...
		foreach($output['files'] as $file) {
			$pqpoutput .=  '<tr><td class="'.$class.'"><b>'.$file['size'].'</b> '.$file['name'].'</td></tr>';
			if($class == '') $class = 'alt';
			else $class = '';
		}
